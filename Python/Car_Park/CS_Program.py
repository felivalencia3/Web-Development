...
def carleave():
    try:
        ...
        if int(hour)-int(entry_time) == 0:
            ...
            charge = 1.50*timedeltat
            if timedeltat >= 8:
                charge = charge + 100
                print("You have stayed over 8 hours and you were charged an additional $100")
                print("\tYou have to pay",charge)
            else:
                print("You have to pay",charge)
    except IndexError:
...
if user == "Client" or user == "c":
    ...
    if welcome == "Entry" or welcome == "e":
        ...
        print("\nWARNING: If you stay for more than 8 hours you will be charged an extra $100")
        accept = input("Go in?\n[Yes/No]\n")
